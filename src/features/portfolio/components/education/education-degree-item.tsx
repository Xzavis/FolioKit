...
import type { EducationDegree } from "@/features/portfolio/types/education"
import { useTranslation } from "@/lib/i18n/use-translation"
import { cn } from "@/lib/utils"

const DEGREE_TYPE_ID: Record<string, string> = {
  "Bachelor's Degree": "Sarjana (S1)",
  "Master's Degree": "Magister (S2)",
  "Doctorate / PhD": "Doktor (S3)",
  "Associate Degree": "Diploma (D3)",
  "High School Diploma": "SMA / SMK",
  "Bootcamp / Academy": "Bootcamp / Akademi",
  "Certification Program": "Program Sertifikasi",
  "Non-Degree / Coursework": "Non-Gelar / Kursus",
}

export function EducationDegreeItem({
  degree,
}: {
  degree: EducationDegree
}) {
  const { t, l, language } = useTranslation()
  const { start, end } = degree.period
  const isOngoing = !end
  const duration = formatDuration(start, end)
  const degreeType =
    language === "id" && degree.degreeType
      ? (DEGREE_TYPE_ID[degree.degreeType] ?? degree.degreeType)
      : degree.degreeType

  return (
    <Collapsible
      className="group/education-degree relative"
      defaultOpen={degree.isExpanded}
      disabled={!degree.description}
    >
      <div
        className="pointer-events-none absolute bottom-0 left-3 hidden size-4 bg-background group-last/education-degree:flex"
        aria-hidden
      >
      ...
      >
        <div className="relative z-1 mb-1 flex items-start gap-3">
          ...
          >
            <IconRegistry name={degree.icon || "graduation-cap"} />
          </div>
        ...
        <div className="flex items-center gap-2 pl-9 text-sm text-muted-foreground">
          {degreeType && (
            <>
              <dl>
                <dt className="sr-only">{t.education.degreeType}</dt>
                <dd>{degreeType}</dd>
              </dl>
              <Separator
          ...
          <dl>
            <dt className="sr-only">{t.education.studyPeriod}</dt>
            <dd className="flex items-center gap-0.5 font-mono text-xs tabular-nums">
              <span>{start}</span>
              <span>-</span>
              <span>{isOngoing ? t.education.present : end}</span>
            </dd>
          </dl>
          ...
          {duration && (
            <>
              ...
              />
              <dl>
                <dt className="sr-only">{t.education.duration}</dt>
      ...
      <CollapsibleContent className="overflow-hidden">
        {degree.description && (
          <Prose className="pt-2 pl-9">
            <Markdown>{l(degree.description, degree.descriptionId)}</Markdown>
          </Prose>
        )}
      </CollapsibleContent>

      {Array.isArray(degree.skills) && degree.skills.length > 0 && (
